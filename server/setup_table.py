...
try:
    ...
    if table_exists:
        ...
        if count > 0:
            print("\n4. Sample data:")
            cursor.execute("SELECT employee_id, full_name, email, role, created_at FROM SalesExecutiveApp_Login LIMIT 3")
            for row in cursor.fetchall():
    ...
    print("="*60)
    print("\nYou can now:")
    print("1. Run: python app.py (to start backend)")
    print("2. Try signup again in browser")
